...
router.get('/accuracy/:vendorId', auth, async (req, res) => {
  try {
    const logs = await PredictionLog.find({ vendor: req.params.vendorId })
      .sort({ createdAt: -1 }).limit(100).lean();

    if (!logs.length) return res.json({ stats: null });
    ...
    res.json({
      stats: {
        ...
        avgErrorMinutes: Math.round(avg * 10) / 10,
        within3MinAccuracy: Math.round(errors.filter(e => e <= 3).length / errors.length * 100),
        within5MinAccuracy: Math.round(errors.filter(e => e <= 5).length / errors.length * 100),
      },
    });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

router.get('/demand-analysis', auth, requireRole('vendor'), async (req, res) => {
  try {
    const since = new Date();
    since.setDate(since.getDate() - 30);

    const orders = await Order.find({
      vendor: req.user.vendorProfile, placedAt: { $gte: since }, status: 'collected',
    }).populate('items.menuItem', 'name category').lean();

    const ordersByHour = {}, ordersByDay = {}, itemsByTimeSlot = {};
    orders.forEach(o => {
      const h = new Date(o.placedAt).getHours();
      const d = ['sun','mon','tue','wed','thu','fri','sat'][new Date(o.placedAt).getDay()];
      const slot = `${h}:00`;
      ordersByHour[slot] = (ordersByHour[slot] || 0) + 1;
      ordersByDay[d] = (ordersByDay[d] || 0) + 1;
      o.items.forEach(i => {
        if (!itemsByTimeSlot[slot]) itemsByTimeSlot[slot] = {};
        const name = i.menuItem?.name || 'Unknown';
        itemsByTimeSlot[slot][name] = (itemsByTimeSlot[slot][name] || 0) + i.quantity;
      });
    });

    const analysis = await geminiService.analyzeDemandPatterns({
      ordersByHour, ordersByDay, itemsByTimeSlot,
    });

    res.json({
      analysis: analysis || { message: 'Not enough data yet' },
      rawStats: { ordersByHour, ordersByDay, totalOrders: orders.length },
    });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
